# Based on the Justfile from https://github.com/urob/zmk-config/blob/main/Justfile
...
draw_config := absolute_path('draw-config')
draw_out := absolute_path('docs/assets')
...
_build expr *west_args: _parse_combos
    #!/usr/bin/env bash
    set -euo pipefail
...
# build firmware for glove80
build:
    just _build glove80
    cat {{ out }}/glove80_*.uf2 > {{ out }}/glove80.uf2
